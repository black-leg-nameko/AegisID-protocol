...
describe('Explicit consent UI', () => {
  it('shows consent page and emits consent_completed on POST', async () => {
    ...
    const authUrl = `${issuer}/auth?client_id=${encodeURIComponent(client_id)}&redirect_uri=${encodeURIComponent(redirect_uri)}&response_type=code&scope=openid&state=${encodeURIComponent(state)}&nonce=${encodeURIComponent(nonce)}&code_challenge=${encodeURIComponent(code_challenge)}&code_challenge_method=S256&force_consent=1`
    const res1 = await fetch(authUrl, { redirect: 'manual', headers: { 'x-correlation-id': 'cid-consent-test' } })
